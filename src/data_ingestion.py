...
def load_data(data_url : str) -> pd.DataFrame:
    ...
    try:
        ...
        logger.error ("Unecpected error has occured while loading the data %s", e)
        raise
...
def preprocess_data(df :pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        logger.error("Missing column in data frame %s", e)
        raise
...
def save_train_test_data(train_data: pd.DataFrame, test_data : pd.DataFrame, data_path: str) -> None:
    '''Save traion and test datasets'''
    try:
        raw_data_path = os.path.join(data_path, "raw")
        os.makedirs(raw_data_path, exist_ok = True)
        train_data.to_csv(os.path.join(raw_data_path, "train.csv"), index = False)
        test_data.to_csv(os.path.join(raw_data_path, "test.csv"), index = False)
        logger.debug("train and test csv data saved to %s", raw_data_path)
